...
# raster mpa no take 
mpa = raster::raster(here::here("data", "processed_data", "predictors", "mpa_type_no_take_only.grd"))
mpa[mpa == -1] <- 0
raster::cellStats(mpa, "sum") # 281 cells in mpa -> corresponding to surface area of 70.25 km2 (one cell = 0.25 km2)
...
#set cost to 0 inside no take mpas
rast_xy <- raster::mask(rast_xy, mpa, updatevalue = 0, maskvalue = 1)
...
for (t in seq(0.1, 0.9, 0.1)){
  ...
  r <- design_reserves_minset_no_penalty_lockin(rast_xy, pred_region, t, mpa)
  
  #map
  png(here::here(paste0(paste0("outputs/prioritisation/map_reserves_minset_no_penalty_lockin_target_", t*100, ".png"))), width = 800, height = 480)
...
calibrate_blm_round2(pred_region, 0.1, seq(0.01, 0.1, 0.01))
#-> selected BLM 0.08

calibrate_blm_round1(pred_region, 0.2)
calibrate_blm_round2(pred_region, 0.2, seq(0.01, 0.1, 0.01))
#-> selected BLM 0.1

calibrate_blm_round1(pred_region, 0.3)
calibrate_blm_round2(pred_region, 0.3, seq(0.01, 0.1, 0.01))
#-> selected BLM 0.02
...
calibrate_blm_round2(pred_region, 0.5, seq(0.01, 0.1, 0.01)) 
#-> selected BLM 0.02

calibrate_blm_round1(pred_region, 0.6)
calibrate_blm_round2(pred_region, 0.6, seq(0.01, 0.1, 0.01))
#-> selected BLM 0.05
...
map_reserves_minset_with_penalty_target_lockin_osm_abundance(maplatlon_proj, pred_region, r, mpas_notake_shp, p, t)

# write raster
raster::writeRaster(r, here::here("data", "processed_data", "prioritisation", paste0("prioritisation_abondance_", t*100)), overwrite=TRUE)
...
reserve_surf_abundance <- c(reserve_surf_abundance_0.1, reserve_surf_abundance_0.2, reserve_surf_abundance_0.3, reserve_surf_abundance_0.4, reserve_surf_abundance_0.5,
...
png(here::here("outputs/prioritisation/plot_reserve_surf_abundance_vs_target.png"), width = 960, height = 960)
par(mar = c(8,8,3,6))
plot(reserve_surf_abundance, targets,
     type = "b",
     las = 2,
