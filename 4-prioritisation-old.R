...
mpa[mpa == -1] <- 0
raster::cellStats(mpa, "sum") # 281 cells in mpa -> corresponding to area of 70.25 km2 (one cell = 0.25 km2)
...
raster::cellStats(pred_region_mpa, 'sum') #72 indiv
raster::cellStats(pred_region_mpa, 'sum') / raster::cellStats(pred_region, 'sum') #7%
...
#t <- design_reserves_minset_no_penalty_lockin(rast_xy, pred_region, t, mpa)
...
raster::cellStats(r, "sum") #1167 cells -> corresponding to area of 291.75 km2 (one cell = 0.25 km2)
...
map_abundance_predictions_region_interp_surv_choosen_prioritisation(maplatlon_proj, pred_region, r, mpas_notake_shp)
